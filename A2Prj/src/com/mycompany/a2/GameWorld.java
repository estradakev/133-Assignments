...
public class GameWorld extends Observable{
	...
	public void init() {
		...
		cyborg = new PlayerCyborg(40, cyborgLocation, ColorUtil.BLUE);
		...
		//Change location to more accurate
		for(int i = 1; i <= 3; i++) {
			gameObjects.add(new NonPlayerCyborg(40,new Point(0,0), ColorUtil.GRAY));
		}
		
		cyborg.setMaximumSpeed(50);
	...
	public void cyborgCollision() {
		...
		if(cyborg.getDamageLevel() < 200 && getLives() > 0) {
			cyborg.setDamageLevel(cyborg.getDamageLevel() + 20);
	...
	private void isGameOver() {
		if(getLives() == 0 && cyborg.getLastBaseReached() < baseObjects) {
			System.out.println("Game over, non-player cyborg wins");
		}
	}
	...
	public void cyborgAccelerate() {
		int speed = cyborg.getSpeed();
		int maxSpeed = cyborg.getMaximumSpeed();
		
		
		if (speed != maxSpeed && maxSpeed != 0) {
			
			if(cyborg.getDamageLevel() <= 50 ) {
				 speed += 5;
				 cyborg.setSpeed(speed);
			}
			else if(cyborg.getDamageLevel() <= 100 ) {
				speed += 3;
				cyborg.setSpeed(speed);
			}
			else if(cyborg.getDamageLevel() <= 150) {
				speed += 1;
				this.cyborg.setSpeed(speed);
			}
			else if(cyborg.getDamageLevel() <= 199 && cyborg.getEnergyLevel() >= 1) {
	...
	public void goRight() {
		...
		System.out.println("The cyborg has turned right by 5 degrees " + this.cyborg.getHeading() + "\n");
	}
	...
	public void mapGame() {
		...
		System.out.println("Cyborg: loc=(" + cyborg.getLocation().getX() + ", " + cyborg.getLocation().getY() + ")"
